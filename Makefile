...
pages-prepare: docs demo-publish
	...
	@for route in binary-conversion number-conversion binary-arithmetic binary-addition twos-complement decimal-to-binary dezimal-zu-binaer binary-to-decimal binaer-zu-dezimal computing-systems operating-systems networking cpu-simulator logic-simulator logic-circuits pla programmable-logic-array hazards glitches moore-machine moore napier-bones normal-forms quine-mccluskey karnaugh-veitch addressing line-coding bitencoding crc subnetting cpu-scheduling realtime-scheduling buddy-memory page-replacement spanning-tree shortest-path tcp process-state; do \
