...
pages-prepare: docs demo-publish
	...
	@for route in binary-conversion number-conversion binary-arithmetic binary-addition twos-complement decimal-to-binary dezimal-zu-binaer binary-to-decimal binaer-zu-dezimal computing-systems operating-systems networking cpu-simulator logic-simulator logic-circuits pla programmable-logic-array hazards glitches moore-machine moore assembler mips napier-bones normal-forms quine-mccluskey karnaugh-veitch addressing line-coding bitencoding crc subnetting cpu-scheduling realtime-scheduling buddy-memory page-replacement spanning-tree shortest-path tcp process-state race-condition race peterson peterson-algorithm pipes pipe ipc interprozesskommunikation virtualisation virtualisierung os-types rechensystemarten os-multiple-choice os-mc layering schichtung cn-topologies topologien twisted-pair verkabelung dns namensaufloesung http mime-types mail smtp email telnet terminal server-identification identifikation networks-multiple-choice networks-mc rechnernetze-mc; do \
